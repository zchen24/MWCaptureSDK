...
static unsigned char g_ascall_map[][16] = 
{
...
void add_text(unsigned char *p_rgb_frame, char *p_text, int x, int y, int w, int h)
{
    int i, j, k;
    ...
    for (k = 0; k < num;) {
        int index = *p_text - 0x20;
        unsigned char *p_map = g_ascall_map[index];
        if (x + 8 > w) {
            x = temp_x;
            y += 16;
            continue;
        ...
        k++;
        p_text++;
        temp_buf = p_rgb_frame + y * 4 * w + x * 4;
        x += 8;
        for (i = 0; i < 16; i++) {
            for (j = 0; j < 8; j++) {
                if (p_map[i] & (1 << (7 - j))) {
