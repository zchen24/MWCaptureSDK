...
void MWOpenGLRender::render_tex(int tex_index, GLenum internalformat, int32_t tex_w, int tex_h, GLenum format, GLenum tex_type, char *p_tex_name, uint8_t *p_data, GLuint glbuffer)
{
    if(glbuffer){
        ...
        glActiveTexture(GL_TEXTURE0 + tex_index);
        glBindTexture(GL_TEXTURE_2D, m_tex[tex_index]);
        glTexImage2D(GL_TEXTURE_2D, 0, internalformat, tex_w, tex_h, 0, format, tex_type, p_data + m_tex_offset[tex_index]);
        glUniform1i(glGetUniformLocation(m_program, p_tex_name), tex_index);
    }
}
...
int32_t MWOpenGLRender::render(uint8_t *p_data, mw_render_ctrl_t *rctrl, int32_t buffers_index)
{
    ...
    if((RENDER_RGB24 == m_render_fourcc) || (RENDER_BGR24 == m_render_fourcc)){
      ...
      (RENDER_ABGR == m_render_fourcc) || (RENDER_ARGB == m_render_fourcc)){
        ...
        /*if(p_data){
            memcpy(m_p_mapbuf[0][0],p_data,m_tex_size[0]);
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, m_width / 2, m_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0);
            glUniform1i(glGetUniformLocation(m_program, "tex_yuv_rgba"), 1);*/
        render_tex(0, GL_RG, m_width, m_height, GL_RG, GL_UNSIGNED_BYTE, "tex_yuv_rg", p_data, m_glbuffers[buffers_index][0]);
        render_tex(1, GL_RGBA, m_width/2, m_height, GL_RGBA, GL_UNSIGNED_BYTE, "tex_yuv_rgba", p_data, m_glbuffers[buffers_index][0]);
        //}
        set_csp = 1;
    }else if((RENDER_NV12 == m_render_fourcc) || (RENDER_NV21 == m_render_fourcc)){
        ...
        render_tex(0, GL_RED, m_width, m_height, GL_RED, GL_UNSIGNED_BYTE, "tex_y", p_data, m_glbuffers[buffers_index][0]);
        render_tex(1, GL_RED, m_width/2, m_height/2, GL_RED, GL_UNSIGNED_BYTE, "tex_v", p_data, m_glbuffers[buffers_index][1]);
        render_tex(2, GL_RED, m_width/2, m_height/2, GL_RED, GL_UNSIGNED_BYTE, "tex_u", p_data, m_glbuffers[buffers_index][2]);
        set_csp = 1;
    }
